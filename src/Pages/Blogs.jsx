  ...
  return (
    <div>
      <div className="py-10 bg-[#fefaf6] relative">
        <div className="max-w-6xl mx-auto px-4 relative pb-24" data-aos="fade-up">
              ...
              <SwiperSlide key={index}>
                <div className="bg-white rounded-xl overflow-hidden flex flex-col md:flex-row shadow-md">
                  ...
                  <div className="flex-1 p-6 md:p-10">
                    <h2 className="text-2xl font-serif md:text-3xl font-semibold text-gray-800 mb-2">
                      {item.title}
                    </h2>
                    <p className="text-sm text-gray-500 font-serif mb-3">{item.date}</p>
                    <p className="text-gray-600 mb-4 font-serif">{item.desc}</p>
                    <a href={item.link} className="text-blue-700 font-serif font-semibold inline-flex items-center gap-1">
                      Read more →
                    </a>
                  </div>
                  ...
                  <div className="flex-1 flex items-center justify-center p-6">
                    <div className="text-center">
                      <p className="text-xl font-semibold mb-4 text-gray-800 font-serif">{item.imgText}</p>
      ...
      <div className="w-full px-4 sm:px-8 md:px-16 lg:px-32 xl:px-48 bg-[#fefaf6] py-6 mx-auto" >
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-6 font-serif" data-aos="fade-up">
          {categories.map((cat) => (
            <button
              key={cat}
              className={`text-sm px-3 py-1 border-b-2 transition ${activeCat === cat ? "border-black font-semibold" : "border-transparent"
                }`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 font-serif" data-aos="fade-up">
          {filteredblogData.map((blog) => (
            <div key={blog.id} className="bg-white border rounded-lg shadow-sm p-3 font-serif">
              <img src={blog.image} alt={blog.title} className="w-full h-48 object-cover font-serif rounded" />
              <h3 className="mt-3 font-semibold text-lg leading-tight font-serif">{blog.title}</h3>
              <p className="text-sm text-gray-500 mt-1 font-serif">{blog.date}</p>
              <p className="text-sm text-gray-600 mt-2 font-serif">{blog.desc}</p>
              <button className="text-blue-600 text-sm mt-3 flex font-serif items-center gap-1">
        ...
        <div className="text-center mt-6" data-aos="fade-up">
          <button className="bg-black text-white px-6 py-2 rounded font-serif hover:bg-gray-800">
            View More
          </button>
        </div>
      </div>
