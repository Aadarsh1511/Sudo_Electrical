...
const projects = [
  {
    icon: <FaShoppingCart className="text-red-600 text-lg font-serif" />,
  ...
  {
    icon: <FaMoneyCheckAlt className="text-red-600 text-lg font-serif" />,
    title: "NepalPay",
  ...
  {
    icon: <FaMapMarkerAlt className="text-red-600 text-lg font-serif" />,
  ...
  {
    icon: <FaUserMd className="text-red-600 text-lg font-serif" />,
    title: "HealthConnect",
  ...
  return (
    <section className="bg-[#f8f4ee] py-16 px-4 md:px-20">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-bold font-serif" data-aos="fade-up">
          <span className="border-red-500 border-b-4 font-serif" >Our</span> <span className="relative inline-block font-serif ">Projects</span>
        </h2>
        <p className="text-gray-600 mt-4 max-w-2xl mx-auto font-serif" data-aos="fade-up">
          Discover our innovative solutions that have transformed businesses across Nepal and beyond.
        </p>
      </div>

      <div className="grid grid-cols-4 gap-6 flex-col pb-4 font-serif" data-aos="fade-up">
        {projects.map((project, idx) => (
          ...
          >
            <img
              src={project.image}
              alt={project.title}
              className="w-full h-48 object-cover rounded-t-xl font-serif"
            />
            <div className="p-4">
              <div className="flex items-center gap-2 mb-2 text-blue-900 font-serif font-semibold">
                {project.icon}
                <h3>{project.title}</h3>
              </div>
              <p className="text-gray-600 text-sm mb-3 font-serif">{project.description}</p>
              <div className="flex flex-wrap gap-2">
                {project.tech.map((tech, i) => (
                  <span
                    key={i}
                    className="bg-gray-200 text-gray-700 text-xs font-serif font-medium px-3 py-1 rounded-full"
                  >
                    {tech}
                  </span>
                ))}
